...
impl Bool_Array {
    pub fn new(mut size: u32) -> Bool_Array // Handling constructor initializer
    {
        Bool_Array {
            _size: size,
            _iteration_number: 1,
        ...
        memset(
            _storage_array,
            0,
            size * mem::size_of_val((_storage_array[0])),
        );
        if option[DEBUG] {
            fprintf(
                stderr,
                ...
                self._size,
                (self._size * mem::size_of_val((_storage_array[0]))) as u32,
            );
        }
    ...
    pub fn set_bit(&mut self, mut index: u32) -> bool {
