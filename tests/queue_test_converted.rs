#![allow(warnings, unused)]
use std::ops::{Add, AddAssign, BitAnd, BitAndAssign, BitOr, BitOrAssign};
use std::ops::{BitXor, BitXorAssign, Div, DivAssign, Index, Mul, MulAssign};
use std::ops::{Neg, Not, Rem, RemAssign, Shl, ShlAssign, Shr, ShrAssign, Sub, SubAssign};
#[path = "../libs/Vector.rs"]
pub mod Vector;
use Vector::{vector, ListInit};

#[path = "../libs/Stack.rs"]
pub mod Stack;
use Stack::stack;

#[path = "../libs/Map.rs"]
pub mod Map;
use Map::map;
#[path = "../libs/Unordered_map.rs"]
pub mod Unordered_map;
use Unordered_map::unordered_map;

#[path = "../libs/Deque.rs"]
pub mod Deque;
use Deque::deque;

#[path = "../libs/Set.rs"]
pub mod Set;
use Set::set;

#[path = "../libs/UnorderedSet.rs"]
pub mod UnorderedSet;
use UnorderedSet::unordered_set;

#[path = "../libs/String.rs"]
pub mod String;
use std::*;
use String::string;
fn main() {
    let mut myQueue = deque::new().clone();
    myQueue.push(1);
    myQueue.push(2);
    myQueue.push(3);
    print!("Front element: {}\n", myQueue.front());
    print!("Back element: {}\n", myQueue.back());
    if myQueue.empty() {
        print!("Queue is empty\n");
    } else {
        print!("Queue is not empty\n");
    }
    print!("Size of the queue: {}\n", myQueue.size());
    let mut anotherQueue = deque::new().clone();
    anotherQueue = myQueue.clone();
    print!("First Queue: ");
    while !myQueue.empty() {
        print!("{} ", myQueue.front());
        myQueue.pop();
    }
    print!("\n");
    print!("Second Queue: ");
    while !anotherQueue.empty() {
        print!("{} ", anotherQueue.front());
        anotherQueue.pop();
    }
    print!("\n");
    myQueue.push(4);
    myQueue.push(5);
    anotherQueue.push(6);
    print!("Before Swap - First Queue: 4,5 Second Queue: 6\n");
    myQueue.swap(&mut anotherQueue);
    print!("After Swap - First Queue: ");
    while !myQueue.empty() {
        print!("{} ", myQueue.front());
        myQueue.pop();
    }
    print!("\n");
    print!("After Swap - Second Queue: ");
    while !anotherQueue.empty() {
        print!("{} ", anotherQueue.front());
        anotherQueue.pop();
    }
    print!("\n");
}
